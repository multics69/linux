...
#define DEV_ENERGY_MODEL_A_PERF_DOMAIN_CPUS_LEN		256

/*************************** Command encoding ********************************/
static int __em_nl_get_pd_size(struct em_perf_domain *pd, void *data)
{
	char cpus_buf[DEV_ENERGY_MODEL_A_PERF_DOMAIN_CPUS_LEN];
	int *tot_msg_sz = data;
	int msg_sz, cpus_sz;
...
static int __em_nl_get_pd(struct em_perf_domain *pd, void *data)
{
	...
	if (nla_put_u32(msg, DEV_ENERGY_MODEL_A_PERF_DOMAIN_PERF_DOMAIN_ID,
			pd->id))
		goto out_cancel_nest;

	if (nla_put_u64_64bit(msg, DEV_ENERGY_MODEL_A_PERF_DOMAIN_FLAGS,
			      pd->flags, DEV_ENERGY_MODEL_A_PERF_DOMAIN_PAD))
		goto out_cancel_nest;

	snprintf(cpus_buf, sizeof(cpus_buf), "%*pb",
		 cpumask_pr_args(to_cpumask(pd->cpus)));
	if (nla_put_string(msg, DEV_ENERGY_MODEL_A_PERF_DOMAIN_CPUS, cpus_buf))
		goto out_cancel_nest;
...
static struct em_perf_domain *__em_nl_get_pd_table_id(struct nlattr **attrs)
{
	...
	if (!attrs[DEV_ENERGY_MODEL_A_PERF_TABLE_PERF_DOMAIN_ID])
		return NULL;

	id = nla_get_u32(attrs[DEV_ENERGY_MODEL_A_PERF_TABLE_PERF_DOMAIN_ID]);
...
static int __em_nl_get_pd_table_size(const struct em_perf_domain *pd)
{
	...
	id_sz = nla_total_size(sizeof(u32));
		/* DEV_ENERGY_MODEL_A_PERF_TABLE_PERF_DOMAIN_ID */
...
int __em_nl_get_pd_table(struct sk_buff *msg, const struct em_perf_domain *pd)
{
	...
	if (nla_put_u32(msg, DEV_ENERGY_MODEL_A_PERF_TABLE_PERF_DOMAIN_ID,
			pd->id))
		goto out_err;
	...
	for (i = 0; i < pd->nr_perf_states; i++) {
		...
		if (nla_put_u64_64bit(msg,
				      DEV_ENERGY_MODEL_A_PERF_STATE_PERFORMANCE,
				      ps->performance,
				      DEV_ENERGY_MODEL_A_PERF_STATE_PAD))
...
static void __em_notify_pd_table(const struct em_perf_domain *pd, int ntf_type)
{
	...
	if (!genl_has_listeners(&dev_energy_model_nl_family, &init_net, DEV_ENERGY_MODEL_NLGRP_EVENT))
		return;
	...
	hdr = genlmsg_put(msg, 0, 0, &dev_energy_model_nl_family, 0, ntf_type);
	if (!hdr)
		goto out_free_msg;
	...
	genlmsg_multicast(&dev_energy_model_nl_family, msg, 0,
			  DEV_ENERGY_MODEL_NLGRP_EVENT, GFP_KERNEL);
...
{
	__em_notify_pd_table(pd, DEV_ENERGY_MODEL_CMD_PERF_DOMAIN_CREATED);
}

void em_notify_pd_updated(const struct em_perf_domain *pd)
{
	__em_notify_pd_table(pd, DEV_ENERGY_MODEL_CMD_PERF_DOMAIN_UPDATED);
}

static int __em_notify_pd_deleted_size(const struct em_perf_domain *pd)
{
	int id_sz = nla_total_size(sizeof(u32)); /* DEV_ENERGY_MODEL_A_PERF_TABLE_PERF_DOMAIN_ID */
...
void em_notify_pd_deleted(const struct em_perf_domain *pd)
{
	...
	if (!genl_has_listeners(&dev_energy_model_nl_family, &init_net,
				DEV_ENERGY_MODEL_NLGRP_EVENT))
		return;
	...
	hdr = genlmsg_put(msg, 0, 0, &dev_energy_model_nl_family, 0,
...
{
	return genl_register_family(&dev_energy_model_nl_family);
}
